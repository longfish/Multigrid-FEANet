...
class MeshCenterInterface():
    """ Handle the square mesh with a central region inside, quad elements
    0: background square; 1: central region
...
class MeshSquare():
    ...
    def generate_global_pattern_map(self):
        """Find nodes associated with each pattern: center nodes and surrounding nodes"""
        self.global_pattern_center[0] = np.ones((self.nnode_edge*self.nnode_edge,),dtype=int) # only one pattern
